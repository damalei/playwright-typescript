...
test.describe('Login', () => {
  ...
  test('[30.1] Log-in using correct user credentials', async ({ page }) => {
    await page.getByLabel('Email Address').fill(`${process.env.SHIPPER_VIZ_VALID_USER}`);
    await page.getByLabel('Password').fill(`${process.env.SHIPPER_VIZ_VALID_PASS}`);
  ...
  test('[30.2] Log-in using incorrect username', async ({ page }) => {
    await page.getByLabel('Email Address').fill(`${process.env.SHIPPER_VIZ_INVALID_USER}`);
    await page.getByLabel('Password').fill(`${process.env.SHIPPER_VIZ_VALID_PASS}`);
    await page.getByRole('button', { name: 'LOG IN' }).click();
    await expect
  ...
  test('[30.3] Log-in using incorrect password', async ({ page }) => {
    await page.getByLabel('Email Address').fill(`${process.env.SHIPPER_VIZ_VALID_USER}`);
    await page.getByLabel('Password').fill(`${process.env.SHIPPER_VIZ_INVALID_PASS}`);
    await page.getByRole('button', { name: 'LOG IN' }).click();
    await expect
      .soft(page.getByText('Wrong email or password'))
      .toBeVisible({ timeout: DEFAULT_TIMEOUT_IN_MS }); // Error Message
