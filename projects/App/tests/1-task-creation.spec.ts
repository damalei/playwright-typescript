...
test('create a task', async ({ page }) => {
  ...
  await page.getByTestId('task-message').locator('textarea').fill(serialnum + ' message')
  ...
  await expect(page.locator("#task-details-title")).not.toBeVisible({ timeout: GLOBALTIMEOUT });
});
...
test('search and open new task as a page', async ({ page }) => {
  ...
  await page.getByTestId('task-card-'+serialnum).click({timeout: GLOBALTIMEOUT})
  ...
  await expect.soft(page.locator("[name='taskReferenceId']")).toHaveValue(serialnum)
