...
test.describe("AP Job Processing - ForwardingShipment", () => {
    ...
    await expect
      .soft(
        page
          ...
          .getByText("DIAO ENG CHAI STEAMSHIP LINE")
      )
      .toBeVisible();
    await expect
      .soft(
        page
          ...
          .getByText("Steamship Line")
      )
      .toBeVisible();
    await expect
      .soft(page.getByText("ForwardingShipmentCargowise"))
      .toBeVisible();
  ...
  test("User edits form then checks the shipment using HBL as reference", async () => {
    ...
    await expect
      .soft(page.getByTestId("check-shipment-info-dialog"))
      .toContainText(
        "Forwarding Shipment with reference no. " +
          shipmentNo +
          " found using HBL No. " +
          hblNo,
    ...
    await expect
      .soft(page.getByTestId("check-shipment-info-dialog"))
      .toContainText(
        "Shipment is associated to the following consolidation/s: " + consolNo
      );
  });
  ...
  test("User clicks show customer modal and verifies reconciliation attempts are created", async () => {
    ...
    await expect
      .soft(page.locator("#notesContainer"))
      .toContainText("Saved reconciliation. Recon attempt ID:");
    await expect
      .soft(page.locator("#notesContainer"))
      .toContainText(
        "Status: Discrepant. Changed external status from for_expedock to todo."
      );
  });
  ...
  test("User edits task name and qa and then verifies changes on task page", async () => {
    ...
    await page
      ...
      .click();
    await page
      .getByRole("option", { name: "imma.damalerio@expedock.com" })
      .click();
    ...
    await expect
      ...
      .toBeVisible();
    await expect
      .soft(page.locator("td:nth-child(5)").first().locator("input"))
      .toHaveValue("imma.damalerio@expedock.com");
  });
});
...
test.describe("AP Job Processing - Customs Declaration", () => {
  ...
  test("User edits form then checks the shipment using HBL as reference", async () => {
    ...
    await page
      .getByTestId("Cargowise Module-shipment-field")
      .getByText("ForwardingShipment")
      .fill("CustomsDeclaration");
    await page.getByTestId("save-and-export-button").click();
    await page.getByTestId("open-check-shipment-info-btn").click();
    await expect
      .soft(page.getByTestId("check-shipment-info-dialog"))
    ...
    await expect
      .soft(page.getByTestId("check-shipment-info-dialog"))
      .toContainText(
        "Customs Declaration with reference no. " +
          shipmentNo +
          " found using HBL No. " +
          hblNo,
        { timeout: 5 * GLOBALTIMEOUT }
...
test.describe("AP Job Processing - ForwardingConsol", () => {
  ...
  test("User edits form then checks the shipment using HBL as reference", async () => {
    ...
    await expect
      .soft(page.getByTestId("check-shipment-info-dialog"))
      .toContainText(
        "Forwarding Consol with reference no. " +
          consolNo +
          " found using HBL No. " +
          hblNo,
        { timeout: 5 * GLOBALTIMEOUT }
...
test.describe("AP Job Processing - ForwardingShipmentConsol", () => {
  let page: Page;
  test.beforeAll(async ({ browser }) => {
    page = await browser.newPage();
    await page.goto(global.testTaskUrl);
  ...
  test("User clicks reconcile from data review modal and verifies Reconciliation Results modal appears", async () => {
    await page.getByTestId("recon-button").click();
    await expect
      .soft(page.getByTestId("ap-invoice-recon-result"))
      .toContainText(
        "Forwarding Consol with reference no. " +
          consolNo +
          " found using HBL No. " +
