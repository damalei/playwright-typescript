...
import { waitDashboardLoad, waitForFilterSectionToLoad, waitforTablePageLoad } from '../../utils';
const __saveFilePath = 'C:/Users/immad/Downloads/'
...
export class Dashboards {
    ...
    constructor(page: Page) {
        this.page = page;
        this.acc_businessPerformance = page.getByTestId('menu-item-business-performance')
    ...
    async exportChart(chartName: string, section: string, dashboard: string) {
        ...
        await this.page.locator(`[data-testid*="${chartName}"]`).getByTestId('download-btn').click()
        const download = await downloadPromise
        await download.saveAs(__saveFilePath + section + dashboard + download.suggestedFilename());
        await this.page.waitForTimeout(60000)
    }

    async exportChartExact(chartName: string, section: string, dashboard: string) {
        const downloadPromise = this.page.waitForEvent('download')
        await this.page.locator(`[data-testid="${chartName}"]`).getByTestId('download-btn').click()
        const download = await downloadPromise
        await download.saveAs(__saveFilePath + section + dashboard + download.suggestedFilename());
      ...
      async downloadByDashboard(dashboardList: string[], section: string){
        ...
        await this.page.locator('span').getByText(`${dashboard}`, {exact: true}).click()
    ...
    async downloadCharts(section: string, dashboard: string, count: number){
        for (let i = 0; i < count; i += 1) {
            ...
            await download.saveAs(__saveFilePath + section + modDashboard + download.suggestedFilename());
          }
    }
}