...
export class ModalAddCard {
    ...
    constructor(page: Page) {
        ...
        this.inputCardName = this.modalAddCard.getByLabel('Card Name').locator('..').locator('..').locator('input')
        this.inputSubtitle = this.modalAddCard.getByLabel('Subtitle').locator('..').locator('..').locator('input')
        this.inputToolTipWhat = this.modalAddCard.getByLabel('What is this report for?').locator('..').locator('..').locator('textarea').first()
        this.inputToolTipHow = this.modalAddCard.getByLabel('How did we get the data?').locator('..').locator('..').locator('textarea').first()
        this.buttonLineChart = this.modalAddCard.getByRole('button', {name: 'LINE CHART'})
        this.inputXaxis = this.modalAddCard.getByLabel('X Axis').locator('..').locator('..').locator('input')
        this.inputYaxis = this.modalAddCard.getByLabel('Y Axis').locator('..').locator('..').locator('input')
        this.buttonSave = this.modalAddCard.getByRole('button', { name: 'Save'})
        this.buttonFilters = this.modalAddCard.getByRole('button', { name: '+ Filters'})
        this.sectionFilters = this.modalAddCard.locator('//h5[text()="Filters"]').locator('..')
        this.buttonEditAdvanceFilter = this.sectionFilters.getByTestId('edit-filters-button')
        this.advanceFilterAccordionShipment = this.sectionFilters.getByTestId('Shipment Filters')
        this.advanceFilterButtonUpdateFilters = this.sectionFilters.getByTestId('update-filters-button')
        this.buttonCancel = this.modalAddCard.getByText('Cancel', { exact: true})
    }

    async setAdvanceTextFilter(page: Page, section: string, rule_field: string, rule_value: string, row_num: number) {
            const accord = this.sectionFilters.getByTestId(`${section}`)
            const ruleField = accord
              .locator('input')
              .nth(row_num);
            await accord.getByRole('button', { name: 'Add rule' }).click()
            await ruleField.click()
            await this.selectDropdownOption(page, `${rule_field}`)
            const valueField = this.sectionFilters.getByTestId(`${section}`)
    ...
    async selectDropdownOption(page: Page, option: string){
        ...
        await dropdownPopper.locator('div').getByText(`${option}`, {exact: true}).click()
    }

    async clickEditCard(page: Page, cardName: string){
        const card = this.page.getByTestId(`data-component-${cardName}`).locator('..').locator('..').locator('..').locator('..')
    ...
    async clearAdvanceFilter(page: Page, section: string, row_num: number){
        const delRow = this.sectionFilters.getByTestId(`${section}`).getByTestId('DeleteIcon').nth(row_num)
        await delRow.click()
