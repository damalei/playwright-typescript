...
export class GlobalFilterSection {
  ...
  constructor(page: Page) {
    ...
    this.advanceFilterContainerAccordion =
      page.getByTestId('Container Filters');
  ...
  async goto() {
    await this.page.goto(
      FREIGHT_BI_BASE_URL +
        '/business-performance/c3f63950-8ce7-4529-b29d-31a92c6f7941'
    );
  ...
  async setBasicTextFilter(
	@@ -59,18 +64,18 @@ export class GlobalFilterSection {
    const boundingBox = this.page.getByTestId(
      `${filtername}-custom-multiple-text-field`
    );
  ...
  async removeBasicTextFilter(filtername: string, el_count: number) {
	@@ -109,14 +114,19 @@ export class GlobalFilterSection {
    );
    let i;
  ...
  async removeAdvanceTextFilter(filtername: string) {
    await this.page
      .locator('span', { hasText: `${filtername}` })
      .locator('//following-sibling::*[1]')
      .click();
  }
