...
test.describe('Clicks on revenue invoices link with org type... ', () => {
  test('Shipper', async ({ page }) => {
    ...
    await waitforTablePageLoad(page, DEFAULT_GLOBAL_TIMEOUT_MS);
    ...
    await exOrg.waitForReferenceComponent();
    const orgRevenue = await exOrg.globalNativeTable.parseCellAmount(
      0,
      'total_revenue'
    );
    await exOrg.globalNativeTable.clickDrillDownLink(
      0,
      'See receivable invoices'
    );
    await waitForFilterSectionToLoad(page, DEFAULT_GLOBAL_TIMEOUT_MS);
    await waitforTablePageLoad(page, DEFAULT_GLOBAL_TIMEOUT_MS);
    const payRevenue = await pay.globalNativeTable.parseSumAmount(
      'amount',
      'SUM'
    );
