...
test.describe.serial('[53] Edit columns on the Organizations page', () => {
  ...
  test('[53.5] User rearranges columns', async () => {
    const expectedHeaderList = await org.globalNativeTable.swapColumns(
      page,
      1,
      2
    );
    newHeaderList = await org.globalNativeTable.dragSourceToTargetColumn(
      page,
      1,
      2
    );
  ...
  test('53.X User removes a column', async () => {
    ...
    await expect
      .soft(
        org.globalNativeTable.columnHeader.getByText(nItemText, { exact: true })
      )
      .not.toBeVisible();
  });
  ...
  test('53.X User adds a column', async () => {
    ...
    await org.globalNativeTable.editColumnButton.click();
    await expect
      .soft(org.globalNativeTable.columnHeader.getByText(addedItem))
      .toBeVisible();
  });
