...
import { waitForElementToHide, waitForFilterSectionToLoad, waitForSnackBar } from '../../utils';
...
test.describe('[28] Dashboard Builder: Create/Edit Dashboards', () => {
    ...
    test('[28.X] Admin user creates a dashboard', async () => {
        ...
        await expect.soft(page.getByTestId('header-title')).toContainText(dashboardTitle)
        await expect.soft(page.getByText('This Dashboard seems empty...')).toBeVisible()
        await expect.soft(page.getByText('Edit the dashboard and add questions to get started')).toBeVisible()
    })
    ...
    test('[28.1] Admin user creates a dashboard with chart that has no card specific filters', async () => {
        ...
        await expect.soft(page.getByTestId(`data-component-${chartName1}`)).toBeVisible({ timeout: DEFAULT_TIMEOUT_IN_MS })
    })
    ...
    test('[28.2] Admin user creates a dashboard with chart with card specific filters', async () => {
        ...
        await dash.ModalAddCard.expandAdvanceFilterAccordion(page, 'Shipment Filters')
        await dash.ModalAddCard.setAdvanceTextFilter(page, 'Shipment Filters', 'Transport Mode', 'AIR', 0)
        await dash.ModalAddCard.buttonSave.click()
        await waitForSnackBar(page, DEFAULT_TIMEOUT_IN_MS)
        await dash.clickLazyLoad(page, chartName2)
        await expect.soft(page.getByTestId(`data-component-${chartName2}`)).toBeVisible({ timeout: DEFAULT_TIMEOUT_IN_MS })
        await dash.ModalAddCard.clickEditCard(page, `${chartName2}`)
        await expect.soft(dash.ModalAddCard.sectionFilters.getByText('Transport Mode is AIR')).toBeVisible()
    })
    ...
    test('[28.4] Admin user duplicates a chart', async () => {
        ...
        await expect.soft(page.getByTestId(`data-component-${chartName2} copy`)).toBeVisible({ timeout: DEFAULT_TIMEOUT_IN_MS })
        await dash.ModalAddCard.clickEditCard(page, `${chartName2}`)
        await expect.soft(dash.ModalAddCard.sectionFilters.getByText('Transport Mode is AIR')).toBeVisible()
    })
    ...
    test('[28.3] Admin user duplicates a dashboard', async () => {
        ...
        await dash.clickLazyLoad(page, `${chartName2}`)
        await dash.clickLazyLoad(page, `${chartName2} copy`)
        await expect.soft(page.getByTestId('header-title')).toContainText(`${dashboardTitle} copy`)
        await expect.soft(page.getByTestId(`data-component-${chartName1}`)).toBeVisible({timeout: DEFAULT_TIMEOUT_IN_MS })
        await expect.soft(page.getByTestId(`data-component-${chartName2}`)).toBeVisible({timeout: DEFAULT_TIMEOUT_IN_MS })
        await expect.soft(page.getByTestId(`data-component-${chartName2} copy`)).toBeVisible({timeout: DEFAULT_TIMEOUT_IN_MS })
        await dash.ModalAddCard.clickEditCard(page, `${chartName2}`)
    ...
    test('[28.5] Admin user edits a duplicated chart', async () => {
        ...
        await dash.ModalAddCard.setAdvanceTextFilter(page, 'Shipment Filters', 'Transport Mode', 'SEA', 0)
        await dash.ModalAddCard.buttonSave.click()
        await waitForSnackBar(page, DEFAULT_TIMEOUT_IN_MS)
        ...
        await expect.soft(dash.ModalAddCard.sectionFilters.getByText('Transport Mode is SEA')).toBeVisible()
        await dash.ModalAddCard.buttonCancel.click()
    ...
    test('[28.6] Admin user creates a custom dashboard with a custom filter and selector set', async () => {
        ...
        await dash.GlobalFilterSection.clickFilterField(
        ...
        await user.inputDashboard('Business Performance', `${dashboardTitle} copy edited`)
        ...
        await expect.soft(page.getByTestId('header-title')).toContainText(`${dashboardTitle} copy edited`)
        await expect.soft(page.locator('label', { hasText: 'Page Last Updated On' })).toBeVisible({ timeout: 10000 });
        await expect.soft(page.getByTestId(`data-component-${chartName1}`)).toBeVisible({timeout: DEFAULT_TIMEOUT_IN_MS })
        await expect.soft(page.getByTestId(`data-component-${chartName3}`)).toBeVisible({timeout: DEFAULT_TIMEOUT_IN_MS })
        await expect.soft(page.getByTestId(`data-component-${chartName3} copy`)).toBeVisible({timeout: DEFAULT_TIMEOUT_IN_MS })
    })
})