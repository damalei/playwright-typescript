...
let rec: ExploreReceivableInvoices;
...
test.describe
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    rec = new ExploreReceivableInvoices(page);
    await rec.goto();
  });
  ...
  test('[51.4] User saves columns', async () => {
    const rec = new ExploreReceivableInvoices(page);
    const expectedHeaderList = newHeaderList;
    await rec.globalFilterSection.saveViewButton.click();
    await rec.globalFilterSection.buttonSaveModal.click();
