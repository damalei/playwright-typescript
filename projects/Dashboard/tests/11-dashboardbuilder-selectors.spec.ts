...
test.describe('Edit and save filters fields on Dashboard Builder', () => {
  ...
  test.afterAll(async ({ browser }) => {
    ...
    await waitForElementToHide(page, DEFAULT_TIMEOUT_IN_MS, '//button[text()="Save"]')
  });
  ...
  test('[11.4] User removes a selector field', async () => {
    ...
    await side.clickOnDashboardName('QA Test Template')
    await waitForFilterSectionToLoad(page, DEFAULT_TIMEOUT_IN_MS);
    await expect.soft(page.locator('//h6[text()="ORG TYPE"]')).not.toBeVisible()
  });

  test.skip('[11.X] User removes all selector fields', {
        annotation: {
    ...
    await waitForFilterSectionToLoad(page, DEFAULT_TIMEOUT_IN_MS);
    await expect.soft(page.locator('//h6[text()="ORG TYPE"]')).not.toBeVisible()
    await expect.soft(page.locator('//h6[text()="PERIOD"]')).not.toBeVisible()
    await expect.soft(page.locator('//h6[text()="CURRENCY"]')).not.toBeVisible()
    await expect.soft(page.locator('//h6[text()="SHIPMENT VOLUME"]')).not.toBeVisible()
    await expect.soft(page.locator('//h6[text()="WEIGHT"]')).not.toBeVisible()
    await expect.soft(page.locator('//h6[text()="VOLUME"]')).not.toBeVisible()
    await expect.soft(page.locator('//h6[text()="GROUP BY DATE"]')).not.toBeVisible()
  });
