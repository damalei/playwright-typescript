...
test('Clicking on Explore Shipment table headers, page should not break', async ({ page }) => {
    ...
    const elements = await exploreShipments.globalNativeTable.getColumnElements()
        for (const element of elements) {
            ...
            await expect.soft(exploreShipments.referenceComponent).toBeVisible({ timeout: DEFAULT_GLOBAL_TIMEOUT_MS })
            let isTableVisible = await exploreShipments.referenceComponent.isVisible()
            if (!isTableVisible) {
...
test('Clicking on Explore Organization table headers, page should not break', async ({ page }) => {
        ...
        const elements = await exploreOrganizations.globalNativeTable.getColumnElements()
            for (const element of elements) {
                ...
                console.log('click' + element)
                await expect.soft(exploreOrganizations.referenceComponent).toBeVisible({ timeout: DEFAULT_GLOBAL_TIMEOUT_MS })
                let isTableVisible = await exploreOrganizations.referenceComponent.isVisible()
                if (!isTableVisible) {
                console.error(`Error on column click: "${headerName}"`)
...
test('Clicking on Payable Invoices table headers, page should not break', async ({ page }) => {
        ...
        const elements = await explorePayableInvoices.globalNativeTable.getColumnElements()
            for (const element of elements) {
                ...
                console.log('click' + element)
                await expect.soft(explorePayableInvoices.referenceComponent).toBeVisible({ timeout: DEFAULT_GLOBAL_TIMEOUT_MS })
                let isTableVisible = await explorePayableInvoices.referenceComponent.isVisible()
                if (!isTableVisible) {
                console.error(`Error on column click: "${headerName}"`)
...
test('Clicking on Receivable Invoices table headers, page should not break', async ({ page }) => {
        ...
        const elements = await exploreReceivableInvoices.globalNativeTable.getColumnElements()
            for (const element of elements) {
                ...
                console.log('click' + element)
                await expect.soft(exploreReceivableInvoices.referenceComponent).toBeVisible({ timeout: DEFAULT_GLOBAL_TIMEOUT_MS })
                let isTableVisible = await exploreReceivableInvoices.referenceComponent.isVisible()
                if (!isTableVisible) {
                console.error(`Error on column click: "${headerName}"`)
