...
#    [Setup]   Run Keyword     Log-in to expedock   passive     ${username}     ${password}
...
#    Click by strategy    label    Branch
...
#    ${el}=  Get Element By    text    HK1BNWBranch
##    ${status}=  Wait For Elements State    ${el}    visible
#    ${count}=   Get Element Count    ${el}
#    Run Keyword If    ${count} == 0     Fail    Missing Sandbox value
...
Assert sandboxing fields
    ...
    ${el}=  Get Element By    text    ${field_text}
    ${count}=   Get Element Count    ${el}
    Run Keyword If    ${count} == 0     Fail    Missing Sandbox value
